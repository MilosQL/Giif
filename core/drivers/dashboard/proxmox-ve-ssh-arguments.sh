...
sshpass -p ${PASSWORD} ssh -q -o ConnectTimeout=5 -o StrictHostKeyChecking=no -p ${PORT} ${USERNAME}@${HOST} 'cat /etc/os-release' 1>/dev/null 2>&1
if [ $? != 0 ] ; then
...
[ -z "$FLOCK" ] && FLOCK=${HOST}
FLOCK=${FLOCK}-${VM_ID}
# Proxmox VE has its own locking mechanism but we are adding this one just in case...
